...
export const productsRouter = t.router({
  getProduct: t.procedure.input(z.string().uuid()).query(async ({ input, ctx: { db } }) => {
  ...
  like: t.procedure.input(z.string().uuid()).mutation(async ({ input, ctx: { db } }) => {
  ...
  dislike: t.procedure.input(z.string().uuid()).mutation(async ({ input, ctx: { db } }) => {
